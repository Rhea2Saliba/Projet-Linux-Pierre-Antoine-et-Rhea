...
if st.session_state.analyzer:
    ...
    # --- CAS 2 : MODE SOLO ---
        ...
        # 1. Préparation des paramètres selon le choix
        args = {}
        ...
        # 3. Recalcul des signaux pour l'affichage et le comptage
        signals = pd.Series(0, index=an.data.index)
        indicator1 = None # Pour Momentum ou BB
        indicator2 = None # Pour Cross MMS (la 2ème courbe)
        label_ind1 = ""
        label_ind2 = ""
        ...
        # 4. Comptage des Ordres
        signals = pd.Series(signals, index=an.data.index)
        positions = signals.diff().fillna(0) # +1 = Achat, -1 = Vente
        
        # On compte combien de fois ce n'est pas 0
        nb_trades = int(positions.abs().sum())

        # 5. Affichage des KPI (Avec le nouveau compteur)
        c1, c2, c3, c4, c5 = st.columns(5) # On ajoute une 5ème colonne
        ...
        c5.metric("Nbr. Ordres", nb_trades, help="Nombre total d'achats et de ventes exécutés")

        # 6. Graphique Avancé (Matplotlib)
        st.subheader(f"📈 Analyse Détaillée : {strat_choice}")
        
        fig, ax = plt.subplots(figsize=(12, 6))
        
        # Prix de l'actif (Noir)
        ax.plot(an.data.index, an.data['Close'], label="Prix Actif", color="black", alpha=0.5, linewidth=1)
        
        # Indicateur 1 (Orange) - Ex: Moyenne Mobile ou MMS Rapide
